...
namespace HEngine
{
	...
	{
		std::optional<std::string> filepath = FileDialogs::OpenFile("HEngine Scene (*.hengine)\0*.hengine\0");
		if (filepath)
		{
			m_ActiveScene = CreateRef<Scene>();
			m_ActiveScene->OnViewportResize((uint32_t)m_ViewportSize.x, (uint32_t)m_ViewportSize.y);
	...
	{
		std::optional<std::string> filepath = FileDialogs::SaveFile("HEngine Scene (*.hengine)\0*.hengine\0");
		if (filepath)
		{
			SceneSerializer serializer(m_ActiveScene);
